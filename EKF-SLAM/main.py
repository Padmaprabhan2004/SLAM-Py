...
class ExtendedKalmanFilterSLAM:
    ...
    def predict(self, control):
        ...
        # where R=V* (covariance in control space)*VTranspose.
        ...
        R3_glob=zeros((2*n+3,2*n+3))
        R3_glob[:3,:3]=R3
        G3_glob=eye(2*n+3)
        G3_glob[:3,:3]=G3
    
        self.covariance = dot(G3_glob, dot(self.covariance, G3_glob.T)) + R3_glob  
        
        self.state[:3] = self.g(self.state[:3], control, self.robot_width)  

    def add_landmark_to_state(self, initial_coords):
        lx,ly=initial_coords[0],initial_coords[1]
        self.state=hstack((self.state,[lx,ly]))
        size=len(self.covariance)
        cov=zeros((size+2,size+2))
        cov[:size,:size]=self.covariance
        cov[-2,-2]=10e10
        cov[-1,-1]=10e10
        self.covariance=cov
    ...
    def h(state, landmark, scanner_displacement):
        ...
        #measurement from lidar to glob frame
    ...
    def correct(self, measurement, landmark_index):
        ...
        n=self.number_of_landmarks
        j=landmark_index
        # set up the full H matrix for EKF-SLAM
        H = zeros((2,len(self.state)))
        #print(H[:,3:5].shape,landmark_index)
        H[:,:3]=H3
        H[:,3+2*j:3+2*j+2]=-H3[:,:2]
...
if __name__ == '__main__':
    ...
    # Read data.
    logfile = LegoLogfile()
    logfile.read("robot4_motors.txt")#control data
    logfile.read("robot4_scan.txt")#lidar data


    with open("ekf_slam_correction.txt", "w") as f:
        for i in range(len(logfile.motor_ticks)):
            ...
            f.write("E %f %f %f %f" % (e + (sqrt(kf.covariance[2,2]),)))
            f.write("\n")
