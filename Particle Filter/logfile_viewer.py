...
class Landmarks(DrawableObject):
    ...
    def background_draw(self):
        for l in self.landmarks:
            if l[0] == 'C':
                x, y = l[1:3]
                ll = to_world_canvas((x - l[3], y - l[3]),
                                     self.canvas_extents, self.world_extents)
                ur = to_world_canvas((x + l[3], y + l[3]),
                                     self.canvas_extents, self.world_extents)
...
def add_file():
    ...
    if filename:
        # If the file is in the list already, remove it (so it will be appended at the end).
...
def load_data():
    ...
    if logfile.filtered_positions:
        if len(logfile.filtered_positions[0]) > 2:
            positions = [tuple(list(to_world_canvas(pos, canvas_extents, world_extents)) + [pos[2]])
...
if __name__ == '__main__':
    ...
    frame2.pack()
    load = tk.Button(frame2, text="Load (additional) logfile", command=add_file)
